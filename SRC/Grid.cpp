...
#include "Grid.h"
 ...
 Grid::Grid(char* fname){
	...
	if(!f1) cout << "Grid data file not found!\n";
	else{
		while(!feof(f1)){
			...
			if(fscanf(f1, "%s", inp) > 0){
				if(!strcmp(inp, "domain")){
					if(fscanf(f1, "%lf %lf", &xrange[0], &xrange[1]) < 0) err++;
					if(fscanf(f1, "%lf %lf", &yrange[0], &yrange[1]) < 0) err++;
				}
				else if(!strcmp(inp, "n")){
					if(fscanf(f1, "%d", &nx) < 0) err++;
					if(fscanf(f1, "%d", &ny) < 0) err++;
				}
				else err++;
